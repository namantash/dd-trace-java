    ...
    if (instrumenter.contextStore().size() > 0) {
          ...
          builder.transform(
              new AgentBuilder.Transformer() {
                final HelperInjector injector =
                    HelperInjector.forDynamicTypes(fieldAccessorInterfaces.values());
                ...
                public DynamicType.Builder<?> transform(
                  ...
                  return injector.transform(
                      builder,
                      typeDescription,
                      // We inject into bootstrap because this is needed by context store
                      // implementations.
                      // Unfortunately this forces us to remove stored type checking because actual
                      // classes
      ...
      builder =
          builder.transform(
              new AgentBuilder.Transformer() {
                final HelperInjector injector =
                    HelperInjector.forDynamicTypes(contextStoreImplementations.values());
